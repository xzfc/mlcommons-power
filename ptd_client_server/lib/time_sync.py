...
def ntp_host_sync(server: str) -> bool:
    try:
        if not validate_remote_time(lambda: get_ntp_time(server)):
            ntp_sync(server)
            return validate_remote_time(lambda: get_ntp_time(server))
    except Exception:
        logging.exception("Could not synchronize with NTP")
        return False
    return True


def remote_host_and_ntp_sync(
    server: str,
    command: Callable[[], float],
    remote_recync: Callable[[], Optional[float]],
) -> bool:
    try:
